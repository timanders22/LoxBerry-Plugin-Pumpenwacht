...
function pw_abo_text()
{
    ...
    if ($f <= 0) {
        return pw_t('LOX.ABO_UNBEKANNT');
    }
    $gemessen = ' <span class="sm-mono">'
              . sprintf(pw_t('LOX.ABO_GEMESSEN'), $f) . '</span>';
    return pw_t($f >= 2 ? 'LOX.ABO_V2' : 'LOX.S2_TEXT') . $gemessen;
}
...
function pw_sicherung_lesen($roh)
{
    ...
    if (!is_array($daten)) {
        return array(null, array(pw_t('EINST.SICH_KEIN_JSON')), 0);
    }
    $neu = pw_vorgaben();
    ...
    foreach ($daten as $k => $w) {
        if (!in_array($k, $bekannt, true)) {
            $mangel[] = sprintf(pw_t('EINST.SICH_FREMD'),
